...
namespace FasterThings.Vehicles.Test1
{
	public class VehicleCamera : MonoBehaviour
	{
		...
		[Header("Components")] public Transform rig;

		[Header("Settings")] public View view = View.Full;
		...
		void UpdateCamera()
		{
			...
			switch (view)
			{
				...
				case View.Full:
					...
					break;
				case View.HalfTop:
					vehicleCamera.rect = new Rect(0, .5f, 1, .5f);
					break;
				case View.HalfBottom:
					vehicleCamera.rect = new Rect(0, 0, 1, .5f);
					break;
				case View.HalfLeft:
					vehicleCamera.rect = new Rect(0, 0, .5f, 1);
					break;
				case View.HalfRight:
					vehicleCamera.rect = new Rect(.5f, 0, .5f, 1);
					break;

				case View.QuarterTopLeft:
					vehicleCamera.rect = new Rect(0, .5f, .5f, .5f);
					break;
				case View.QuarterTopRight:
					vehicleCamera.rect = new Rect(.5f, .5f, .5f, .5f);
					break;
				case View.QuarterBottomLeft:
					vehicleCamera.rect = new Rect(0, 0, .5f, .5f);
					break;
				case View.QuarterBottomRight:
					vehicleCamera.rect = new Rect(.5f, 0, .5f, .5f);
					break;
		...
		void FixedUpdate()
		{
			...
			rig.position = Vector3.Lerp(rig.position, transform.position + cameraPositionOffset,
				Time.deltaTime * followSpeed);
			if (followRotation)
			{
				rig.rotation = Quaternion.Lerp(rig.rotation,
					Quaternion.Euler(transform.eulerAngles + cameraRotationOffset), Time.deltaTime * rotationSpeed);
			}
